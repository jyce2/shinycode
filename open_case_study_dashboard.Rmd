...
# Import data set from wd(), load .rda file
setwd("C:/Users/guestuser/OneDrive - University of North Carolina at Chapel Hill/qual/code")
load("shooting_data_wrangled_pre_map.rda")
load("shooting_data_wrangled_for_map.rda")
...
w <-deaths_perc_event %>% 
  ...
  mutate(percent = ceiling(percent)) %>%
  select(-num_events) %>%
  arrange(percent)
  
  
ggplot(data=w, aes(fill=category, values=percent)) + 
    geom_waffle( color = "white", make_proportional = TRUE) + scale_fill_viridis_d() + coord_equal() + theme_void() +  
...
ggplot(data=wa, aes(fill=category, values=percent)) + 
    geom_waffle( color = "white", make_proportional = TRUE) + scale_fill_viridis_d() + coord_equal() + theme_void() +  
    labs(title = "Deaths Per School Shooting", 
         subtitle = "1 square ~ 1%")
